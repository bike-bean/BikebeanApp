...
public class MapElements {
    ...
    public MapElements(final @NonNull GoogleMap googleMap,
        ...
        marker = new BikeMarker(googleMap, currentPositionBike, areaColor);
    }
    ...
    @NonNull CameraUpdate getCameraUpdate(final @NonNull Marker marker, final double offset) {
        final @NonNull LatLng markerPosition = marker.getPosition();
        return CameraUpdateFactory.newLatLng(new LatLng(
                markerPosition.latitude - offset, markerPosition.longitude
        ));
    }

    @NonNull CameraUpdate getCameraUpdate(final @NonNull Circle circle, final double offset) {
        final @NonNull LatLng circleCenter = circle.getCenter();
        return CameraUpdateFactory.newLatLng(new LatLng(
                circleCenter.latitude - offset, circleCenter.longitude
        ));
    }
    ...
    static class AreaColor {
        ...
        private final @NonNull @ColorInt Integer mainColor;  /* the color of the marker */
        ...
        AreaColor(final @NonNull MapFragmentViewModel mf, final @NonNull Context context,
            ...
            daysSinceLastState = DateUtils.getDaysSinceState(args);
            ...
            if (menuItemId == R.id.menu_satellite || menuItemId == R.id.menu_hybrid) {
                mainColor = ResourceUtils.getMarkerColor(context, daysSinceLastState);
                secondaryColor = ContextCompat.getColor(context, R.color.white);
            } else if (menuItemId == R.id.menu_normal)
                mainColor = secondaryColor =
                        ResourceUtils.getMarkerColor(context, daysSinceLastState);
            else
                mainColor = secondaryColor =
                        ContextCompat.getColor(context, R.color.brandColorGreen);
        }

        @NonNull BitmapDescriptor getMarkerIcon() {
            final @Nullable Bitmap bitmap =  mf.getMapMarkerBitmap(
                    MapMarkerCache.DRAWABLES.mapMarker,
                    mainColor
            );
        ...
        void refreshColor(final @NonNull State state) {
            daysSinceLastState = DateUtils.getDaysSinceState(state);
        ...
        void refreshColor(int menuItemId) {
            if (menuItemId == R.id.menu_satellite || menuItemId == R.id.menu_hybrid)
                ...
                secondaryColor = ResourceUtils.getMarkerColor(context, daysSinceLastState);
            else
                secondaryColor = ContextCompat.getColor(context, R.color.brandColorGreen);
        }
    }
